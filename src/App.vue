<template>
    <div class="app" @click="click">
        ...
        >
            <span v-if="currentMode === modes.talents"
                >Hover a talent to&nbsp;see&nbsp;it's&nbsp;frequency</span
            >
            <span v-if="currentMode === modes.items"
                >Hover an item to&nbsp;see&nbsp;it's&nbsp;frequency</span
            >
            <span v-if="currentMode === modes.stats"
                >Frequent&nbsp;stats&nbsp;priority diplayed on&nbsp;top
                of&nbsp;the&nbsp;characters&nbsp;list</span
            >
        </ProgressBar>
    </div>
</template>
...
import LegendariesTab from "./features/LegendariesTab";
        ...
        hoveredSpellFrequency() {
            ...
            if (this.currentMode === modes.items) {
                charsUseSpells = this.chars.map((char) =>
                    char.items.some(
                        (item) =>
                            item.id === this.hoveredItem.id ||
                            (item.spells || []).includes(this.hoveredItem.id),
                    ),
                );
            }
